...
def bg_holidays(year):
    holidays = {date(year,1,1),date(year,3,3),date(year,5,1),date(year,5,6),date(year,5,24),date(year,9,6),date(year,9,22),date(year,11,1),date(year,12,24),date(year,12,25),date(year,12,26)}
...
def init_db():
    with get_db() as db:
        ...
        if not db.execute("SELECT COUNT(*) FROM users").fetchone()[0]:
            ...
            td = timedelta
            seeds = [
                ('Монтаж на палуба','critical','progress',2,1,today+td(days=4),[3,4]),
                ('Проверка OBC зарядни','high','todo',3,1,today+td(days=2),[]),
                ('Инвентаризация секция Б','medium','todo',4,4,today+td(days=6),[5]),
                ('Подготовка документи','low','review',5,2,today+td(days=9),[]),
                ('Тест електрическа система','critical','done',6,3,today+td(days=1),[2,3]),
                ('Поръчка липсващи крепежи','high','progress',2,4,today+td(days=3),[]),
                ('Лакиране корпус','medium','todo',3,1,today+td(days=8),[6]),
                ('Качване данни в системата','low','done',4,5,today+td(days=5),[]),
                ('Преглед доставка от Италия','high','progress',5,4,today+td(days=2),[2]),
                ('Настройка навигация','medium','review',6,3,today+td(days=7),[]),
                ('Монтаж хардуер борд','high','todo',2,2,today+td(days=5),[]),
                ('Тест водоустойчивост','critical','progress',3,1,today+td(days=3),[4]),
            ]
            for s in seeds:
                title, pri, status, aid, pid, due, collabs = s
                r = db.execute("INSERT INTO tasks (title,priority,status,assignee_id,project_id,due_date,created_by) VALUES (?,?,?,?,?,?,1)",
                               (title, pri, status, aid, pid, due))
                tid = r.lastrowid
                for cid in collabs:
                    db.execute("INSERT INTO task_collaborators (task_id, user_id) VALUES (?,?)", (tid, cid))
            yd = today - td(days=1)
            rpt_data = [
...
async def create_task(request: Request):
    """Any authenticated user can create tasks and assign them to anyone."""
    user = current_user(request)
    if not user: return JSONResponse({"error": "unauthorized"}, 401)
    ...
    with get_db() as db:
        r = db.execute("INSERT INTO tasks (title,description,priority,status,assignee_id,project_id,due_date,created_by) VALUES (?,?,?,?,?,?,?,?)",
            (data["title"], data.get("description",""), data.get("priority","medium"), data.get("status","todo"), aid, data.get("project_id"), data.get("due_date"), user["id"]))
        tid = r.lastrowid
        seen = set()
...
async def delete_task(request: Request, tid: int):
    """Allow CEO/Manager OR creator of the task to delete it."""
    user = current_user(request)
    if not user: return JSONResponse({"error": "unauthorized"}, 401)
    with get_db() as db:
...
async def create_project(request: Request):
    user = current_user(request)
    if not user: return JSONResponse({"error": "forbidden"}, 403)
    data = await request.json()
    with get_db() as db:
